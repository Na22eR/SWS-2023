...
import { initCanvas } from "../canvas/init.js";
 ...
 */
function handleRegisteredForCanvas(registeredEvent) {
 ...
 * @param roomEvent
 */
function handleCanvasChangedEvent(roomEvent) {
    if (canvas) {
 ...
 * @param roomEvents
 */
function handleGetCanvasEventsResponse(roomEvents) {
    ...
    if (roomEvents.canvasId === getCurrentCanvasRoom()) {
        ...
        if (blockedShapes.size > 0) {
            for (let shapeId of blockedShapes.keys()) {
                ...
                // then the shape is added to the blocked shape list. Otherwise,
                // the shape is added to the selected list
...
/**
 * Checks if the passed roomId is in the openRooms list.
